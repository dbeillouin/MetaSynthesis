...
###### Load Data
 ...
 TAB<-dplyr::bind_rows(BEST_fit_DO2_LU_IN_SubIN,DATABASE_un)%>%
    ...
    dplyr::mutate(Intervention = factor(Intervention,
                                      levels=rev(c("land use change",
                                                     "management",
                                                     "global changes")))) %>%
    dplyr::mutate(estimate = (exp(estimate)-1)*100)  %>%
    tidyr::separate(Sub_Cat_intervention, c("first", "second"),sep='->') %>%
    mutate(Land_use = factor(ifelse(Land_use == "land_use change", first, Land_use)))%>%
...
p<-ggplot(TAB,
  ...
  scale_fill_manual(values = c("darkgreen", "orange", "green" ,'gray',"blue","yellow"))+
  ylim(c(-100,120))+
 ...
 plot<-cowplot::ggdraw() +
  cowplot::draw_plot(p) +
  cowplot::draw_plot(plot_Land, x = 0.85, y = 0.2, width = 0.2, height = 0.2)+
   cowplot::draw_plot(Plot_INT, x = 0.85, y = 0.7, width = 0.2, height = 0.2)
